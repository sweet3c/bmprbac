...
CREATE TABLE `bmp_user` (
  ...
  `gender` tinyint(3) NOT NULL DEFAULT '-1' COMMENT '性别：0为女，1为男，-1为保密',
  `status` tinyint(3) unsigned NOT NULL DEFAULT '1' COMMENT '在职状态：1在职，2离职',
...
CREATE TABLE `rbac_authitems` (
  `item_name` varchar(64) NOT NULL COMMENT '权限完整名称，包含mondule,controller,action',
...
) ENGINE=InnoDB AUTO_INCREMENT=16 DEFAULT CHARSET=utf8 COMMENT='rbac任务表';
...
) ENGINE=InnoDB AUTO_INCREMENT=12 DEFAULT CHARSET=utf8 COMMENT='任务分类表';
